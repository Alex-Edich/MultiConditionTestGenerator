...
function activate(context) {
    ...
    function readMethodContent() {
        ...
        if (editor) {
            ...
            while (token !== typescript_1.default.SyntaxKind.PublicKeyword) {
                scanner.scan();
                token = scanner.getToken();
                if (scanner.getToken() === typescript_1.default.SyntaxKind.EndOfFileToken) {
            ...
            do {
                ...
                scanner.scan();
            } while (bracketCounter !== 0 && scanner.getToken() !== typescript_1.default.SyntaxKind.EndOfFileToken);
        }
     ...
     * @returns
     */
    function createParameterNameTable(parameterTypes, parameterNames) {
        let parameterTable = `
			<td>
				parameters
			</td>
     ...
     * @returns
     */
    function createSortedValuesTable(parameterNames, sortedValues) {
    ...
    function createTruthTable(sortedValues, fullTruthTable, possibleCases) {
        ...
        sortedValues.forEach(parameterConditions => {
            ...
            for (let conditionCell = 1; conditionCell < parameterConditions.length; conditionCell++) {
                ...
                let condition = parameterConditions[0][0];
    ...
    function createCaseCreationTable(parameterNames, fullTruthTable, possibleCases) {
        let inputNum = 0;
        let table = `	<table class="content">
							<tr>
    ...
    function conditionToTokens(condition) {
        let tokens = [];
        let token = "";
    ...
    function allFullConditionsToTokens(fullConditions) {
        let conditionTokens = [];
    ...
    function splitMultiToSingleCondition(fullConditionTokens) {
        let singleConditions = [];
        let currentCondition = 0;
        let currentConditionTokens = [];
    ...
    function splitAllMultiToSingleConditions(fullConditions) {
     ...
     */
    function sortValues(parameterNames, singleConditions) {
        let sortedValues = [];
    ...
    function findPossibleCases(fullTruthTable) {
        let possibleCases = [];
        for (let testCase = 0; testCase < fullTruthTable.length; testCase++) {
            let valid = true;
     ...
     * @returns
     */
    function combineFullTruthTable(parameterTypes, parameterNames, singleConditions) {
        ...
        let fullTruthTable = getParameterCombinationRange(parameterTypes[parameterNames.length - 1], sortedValues[parameterNames.length - 1]);
        for (let parameter = parameterNames.length - 2; parameter >= 0; parameter--) {
            let parameterTruthTable = getParameterCombinationRange(parameterTypes[parameter], sortedValues[parameter]);
     ...
     */
    function combineTruthTables(table1, table2) {
        let allCasesWithRanges = [];
     ...
     */
    function getParameterCombinationRange(parameterType, conditions) {
        //create truth table with all possible combinations
        let truthTable = []; //[[false, false, false], [false, false, true], ...];
        ...
        for (let tableCase = 0; tableCase < truthTable.length; tableCase++) {
            let caseRange = [new ValueRange(true, -Infinity, Infinity, true)];
            for (let condition = 1; condition < conditions.length; condition++) {
                let conditionRange = getRange(conditions[condition], truthTable[tableCase][condition - 1]);
    ...
    class ValueRange {
        ...
        maxEdge;
        constructor(minEdge, start, end, maxEdge) {
     ...
     */
    function getRange(condition, isConditionTrue) {
     ...
     */
    function getRangeIntersection(datatype, range1, range2) {
        switch (datatype) {
            case "int":
            case "double":
            case "short":
    ...
    //MARK: Popup content
    function getWebviewContent(functionName, returnType, parameterNames, parameterTypes, fullConditions) {
        const singleConditions = splitAllMultiToSingleConditions(fullConditions);
        const sortedValues = sortValues(parameterNames, singleConditions);
    ...
    //MARK: Command: Popup
    let sendParametersToPopup = vscode.commands.registerCommand('mctg.sendParametersToPopup', (uri) => {
        ...
        const returnType = methodContent[0];
        const functionName = methodContent[1];
        const parameterTypes = methodContent[2];
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            if (message.command === 'createTestFile') {
                ...
                const testFilePath = path.join(vscode.workspace.rootPath || '', testRelativePath, path.basename(uri.fsPath)).replace("\.java", "Test\.java");
                const singleConditions = splitAllMultiToSingleConditions(fullConditions);
                const truthTable = combineFullTruthTable(parameterTypes, parameterNames, singleConditions);
                const possibleCases = findPossibleCases(truthTable);
                const inputsPerCase = inputs.length / possibleCases.length;
                let testStrings = [];
                for (let testCase = 0; testCase < possibleCases.length; testCase++) {
                    ...
                    if (isEveryInputSet) {
                        ...
                        const testString = createTestString(testName, returnType, functionName, parameterTypes, parameterNames, parameterTestValues, mainFile);
...
async function createTestFile(testFilePath, testStrings) {
    ...
    if (!fs.existsSync(testFilePath)) {
        ...
        const testFile = fileNodes[fileNodes.length - 1].replaceAll("\.java", "");
        const mainFile = testFile.replace("Test", "");
        const classString = `//Auto-Generated
